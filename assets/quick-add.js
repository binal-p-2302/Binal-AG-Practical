...
function updateData() {
  document.querySelectorAll('quick-add-modal .qty-btn-plus, quick-add-modal .qty-btn-minus').forEach(button => {
    button.addEventListener('click', (event) => {
      const qtyInput = button.closest('.variant-qty-box').querySelector('.input-qty');
        ...
        if(closestTr){
          ...
          if(variantTotal){
            ...
            if(discountData){
              ...
              let foundObj = discountArr?.reduce((prev, current) => {
                  return current.qty <= updatedQty && current.qty > prev.qty ? current : prev;
              }, { qty: -Infinity });
 
              let discount = foundObj?.discount;
              if(Number(discount) > 0){
                variantTotalPrice = variantTotalPrice - (variantTotalPrice * (discount / 100));
              }
              let variantDiscount = closestTr.querySelector(".variant-discount");
              if(variantDiscount){
                variantDiscount.innerHTML = `${discount}%`;
              }
            }

            closestTr.querySelectorAll("[data-total-price]").forEach(function (element) {
              element.setAttribute("data-total-price", variantTotalPrice);
            })

            variantTotal.innerHTML = Shopify.formatMoney(variantTotalPrice, '${{amount}}');
            
            let quickViewVariantTable = button.closest(".quick-view-variant-table");
            if(quickViewVariantTable){
              ...
              if(productInfo){
                ...
                let totalItemCount = productInfo.querySelector(".total-item-count");
                if(totalItemCount){
                ...
                if(Number(totalQty) > 0){
                  ...
                  if (submitBtn && !submitBtn.hasAttribute('disabled')) submitBtn.setAttribute('disabled', 'disabled');
                }
                let totalPrice = 0;
                quickViewVariantTable.querySelectorAll("[data-total-price]").forEach(function (element) {
                  totalPrice += Number(element.dataset.totalPrice);
                })
                let totalItemPrice = productInfo.querySelector(".total-item-price");
                if(totalItemPrice){
  ...
  customElements.define(
    ...
    class QuickAddModal extends ModalDialog {
      ...
      show(opener) {
        ...
        let url = `${opener.getAttribute('data-product-url')}?view=quick-drawer`;
